...
// and AuditReportPanel (Component D, its own full test suite already) are
// stubbed so this doesn't depend on their internals or data-fetching.
...
describe("ReportsPage", () => {
  ...
  it("still shows the mock Disposal tab", async () => {
    getMeMock.mockResolvedValue(ADMIN);
    const user = userEvent.setup();
    render(<ReportsPage />);

    await user.click(screen.getByRole("tab", { name: "Disposal" }));
    // Maintenance's mock text used to be ambiguous with Disposal's here
    // (same generic MockNotice title); now that Maintenance is real, this
    // checks Disposal's own description text directly.
    expect(screen.getByText(/Every disposal outcome/)).toBeInTheDocument();
  });
});
