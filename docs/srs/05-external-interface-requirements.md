...
| Model provider, via `IModelClient` | API → Azure OpenAI / OpenAI / Anthropic / on-prem model (configured per deployment) | Outbound-only HTTPS call from the Planner node — the one node whose criteria (§7.2.1) require a model. Provider API key held in server configuration (AI-26), never transmitted to a client. The other three agent nodes have no external interface here at all — they run in-process, invoked directly by the Orchestrator, and never leave the API process. | Timeout of 120 seconds for the overall run (AI-25); on timeout, rate-limit or error the workflow is recorded as a safe failure with the cause, and no business state changes. |
| Agent tools | In-process, node → tool service (§7.2.1, §7.4) | Each `IAgentNode` is constructed only with the typed tool-service interfaces its own allow-list permits (enforced at dependency-injection registration, not an HTTP gateway); every tool call is still schema-validated on input and output (AI-01/AI-02) and logged with agent, input hash, outcome, duration and retry count (AI-07), identically to an out-of-process call. The `/api/agent-tools/*` HTTP endpoints (§7.4) remain for any agent implementation still running out-of-process (§7.3's "current implementation" column) and continue to authenticate as the agent service principal. | A rejected or malformed tool call returns a structured error the node must handle; repeated failure routes the workflow to safe failure. |
