...
CoreGrid is a new, self-contained product rather than a replacement component within an existing system. It comprises five cooperating parts: an ASP.NET Core Web API that is the sole authoritative application layer; a PostgreSQL relational database; a React single-page application used as the management and control centre; a Flutter mobile application used for field operations; and an agentic-AI subsystem that executes the agentic workflow, reachable only internally. An in-process Agent Orchestrator inside the API sequences four agent nodes, all running in the same deployable; only the Planner node calls a model, through a provider-agnostic interface so the model vendor (Azure OpenAI, OpenAI, Anthropic, or an on-prem model) is a deployment-time configuration choice, not a code dependency — see Section 3.6 and SRS §7.2.1.
...
| Agentic-AI subsystem | Target: no separate runtime — the Orchestrator and all four agent nodes run in-process within the API (Section 7.2.1, ADR-010). Current: the Planner node still runs as a standalone Python/LangGraph process (`planner-agent/`), reachable only from the API over a private network path or a shared-secret-authenticated internal endpoint, no public ingress, pending migration in-process; Maintenance Analysis, Budget Analysis and Policy Compliance have no separate runtime (Budget's prior standalone implementation was removed once the target design made it redundant — see `docs/progress.md`). |
