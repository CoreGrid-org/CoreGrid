...
| Database entities owned (§8.2) | Needed so that a schema change (see the physical design in [`appendix-f-physical-database-schema.md`](appendix-f-physical-database-schema.md)) is reviewed by the right person before merge. |
...
- **Component ownership never changes; delivery status does.** [`docs/progress.md`](../progress.md) is the up-to-date record of what each owner has actually landed against the requirement range in §18.2; this file records who is accountable, not what is finished. If progress.md shows an item as ❌ past the week it was due in §18.9, that is a schedule risk to raise, not a roster edit.
