...
| Medium | Shared multi-tenant SaaS delivery with per-tenant plans and billing, as an alternative to today's one-deployment-per-department model. | Every entity is already organisation-scoped with a global query filter (Section 4.5); a shared-SaaS pivot would additionally need per-department identity-provider organisations, which the current self-hosted, single-tenant-per-deployment design deliberately does not have (Section 4.2). |
