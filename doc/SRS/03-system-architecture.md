...
| Agentic AI | Python LangGraph for the Budget Analysis Agent; .NET-native for the remaining agent nodes | LangGraph was originally selected because the assignment's acceptance criteria map directly onto its primitives: an explicit graph of distinct nodes, a typed shared state object, checkpointed persistence, conditional edges for validation-driven routing, and a first-class interrupt mechanism for human approval. Recorded in ADR-005. The Policy Compliance Agent was then built .NET-native, in-process within the API, with a deterministic rule engine standing in for the model call — this proved the acceptance criteria don't require a separate Python runtime, so team direction as of 2026-09-14 is to build the remaining nodes (Planner, Maintenance Analysis) the same way. The already-built Budget Analysis Agent keeps its Python/LangGraph implementation as-is. |
