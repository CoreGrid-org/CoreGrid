...
| Identity | ThunderID (OIDC / OAuth 2.0) | Removes credential storage from CoreGrid entirely, provides the organisation construct the platform model requires, and supplies standards-based tokens the API validates with published keys. Recorded in ADR-002. |
