...
| 2 | Record the organisation unit's identifier. Each customer organisation's deployment is self-hosted and single-tenant (M0, Section 4.2), so no sub-organisation is created — there is only ever the one, default organisation unit. | The recorded identifier matches `ThunderID:OuId` in the backend configuration. |
...
| 5 | Record the identifier of ThunderID's built-in `System` resource server (no new resource server is created — its `system` permission is what the built-in Administrator role already grants). This is `ThunderID:Resource`. | The `aud` claim in a test token matches this identifier. |
...
| 7 | Register a confidential machine-to-machine application for SCIM user provisioning and grant it the `system` scope. | A test invitation creates a user in the deployment's single organisation unit. |
