...
| Agentic-AI subsystem | Target: no separate runtime — the Orchestrator and all four agent nodes run in-process within the API (Section 7.2.1, ADR-010). Current: the Planner node still runs as a standalone Python/LangGraph process (`planner-agent/`), reachable only from the API over a private network path or a shared-secret-authenticated internal endpoint, no public ingress, pending migration in-process; Maintenance Analysis, Budget Analysis and Policy Compliance have no separate runtime (Budget's prior standalone implementation was removed once the target design made it redundant — see `doc/PROGRESS.md`). |
