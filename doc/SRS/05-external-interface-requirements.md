...
|---|---|---|---|
| Asgardeo — OIDC authorisation | Clients → Asgardeo | Authorisation Code with PKCE; discovery document; JWKS. | Authentication failure returns the user to sign-in with a non-technical message. JWKS retrieval failure serves the cached key set and raises a health-check warning. |
| Asgardeo — SCIM 2.0 | API → Asgardeo | REST/JSON with a confidential service credential, for user creation, role assignment and deactivation. | Timeout of 10 seconds; two retries with exponential backoff; on final failure the administrator is told the invitation could not be sent and no partial local state is committed. |
